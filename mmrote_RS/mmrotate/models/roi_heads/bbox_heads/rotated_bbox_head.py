...
class RotatedBBoxHead(BaseModule):
    ...
    def loss(self,
             ...
             label_weights,  # tensor([1., 1., 1.,  ..., 1., 1., 1.], device='cuda:0')
             bbox_targets,
             bbox_weights,
             reduction_override=None,flag = False):
        """Loss function.
        ...
        if not flag:
            if cls_score is not None:
                avg_factor = max(torch.sum(label_weights > 0).float().item(), 1.) # (num_proposals,)
    ...
    def get_bboxes(self,
        ...
        if cfg is None:
            ...
            if large:
                det_bboxes, det_labels,select, box_cls = multiclass_nms_rotated(
                    bboxes, scores, cfg.score_thr, cfg.nms, cfg.max_per_img,large=large)
                return det_bboxes, det_labels, select, box_cls
            else:
