...
class RotatedStandardRoIHead(BaseModule, metaclass=ABCMeta):
    ...
    def _bbox_forward(self, x, rois,flag = False):
    ...
    def simple_test(self, x, proposal_list, img_metas,large = False, rescale=False):
        ...
        if large:
            ### labels 0-25
            det_bboxes, det_labels, selec_cls_scores, all_box_cls = self.simple_test_bboxes(
                x, img_metas, proposal_list, self.test_cfg, large =large ,rescale=rescale)
            
            #### check det_labels, selec_cls_scores
            # for y1,y2 in zip(det_labels, selec_cls_scores):
            #     for yy1,yy2 in zip(y1,y2):
            #         if not int(np.argmax(np.array(yy2.cpu()))) == yy1:
            #             print(0)
            #### 
            bbox_results = [
            ...
            new_selec = [
                rbbox2result(selec_cls_scores[j], det_labels[j],
                            self.bbox_head.num_classes)
                for j in range(len(det_bboxes))
            ]

            new_all_box_cls = [
                rbbox2result(all_box_cls[j], det_labels[j],
                            self.bbox_head.num_classes)
                for j in range(len(det_bboxes))
            ]
            ...
            # new_selec = []
            # for i in range(len(det_bboxes)):
            #     new_selec.append([selec_cls_scores[i][det_labels[i] == i, :] for i in range(self.bbox_head.num_classes)])
            # #### 更新 selec_cls_scores
            # new_selec = [[[] for ll1 in range(26)] for ll in range(len(bbox_results))]
            # for kk in range(len(bbox_results)):
            #     # new_selec.append([])
            #     glag = 0
            #     non_empty_indices = [idx for idx, arr in enumerate(bbox_results[kk]) if arr.size > 0]
            #     # for tem,id in zip(bbox_results[kk],det_labels[kk]):
            #     for tem in zip(bbox_results[kk],det_labels[kk]):


            #         if len(tem) == 0:
            #             continue
            #             # new_selec[kk][id].append([])
            #         else:
            #             lens = len(tem)
            #             if glag == 0:
            #                 new_selec[kk][id].append(selec_cls_scores[kk][0:lens,:])
            #                 glag = glag + lens
            #             else:
            #                 new_selec[kk][id].append(selec_cls_scores[kk][glag: glag +lens,:])
            # for jj in range(len(new_selec)):
            #     for jj1 in range(26):
            #         if len(new_selec[jj][jj1]) != 0:
            #             new_selec[jj][jj1] = torch.cat(new_selec[jj][jj1])
            ...
            return bbox_results,new_selec,new_all_box_cls
