...
from .utils_relation import nms_overlaps_rotated
...
class DecoderRNN(nn.Module):
    ...
    def forward(self, inputs, initial_state=None, labels=None, boxes_for_nms=None,OBB=  None):
        ...
        if boxes_for_nms is not None and not self.training:
            if OBB:
                is_overlap = nms_overlaps_rotated(boxes_for_nms).view(
...
class LSTMContext(nn.Module):
    ...
    def obj_ctx(self, obj_feats, proposals, obj_labels=None, boxes_per_cls=None, ctx_average=False,OBB = None):
        """
    ...
    def forward(self, x, proposals, rel_pair_idxs, logger=None, all_average=False, ctx_average=False):
        ...
        if proposals[0].bbox.shape[-1] == 5: 
            ...
            OBB = True
            # box_info = encode_orientedbox_info(proposals)
        else:
        ...
        obj_dists, obj_preds, obj_ctx, perm, inv_perm, ls_transposed = self.obj_ctx(obj_pre_rep, proposals, obj_labels, boxes_per_cls, ctx_average=ctx_average,OBB=OBB)
