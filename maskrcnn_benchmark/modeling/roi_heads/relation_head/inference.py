...
class PostProcessor(nn.Module):
    ...
    def forward(self, x, rel_pair_idxs, boxes):
        ...
        for i, (rel_logit, obj_logit, rel_pair_idx, box) in enumerate(zip(
            ...
            if self.use_gt_box: #  change
                obj_scores, obj_pred = obj_class_prob[:, 1:].max(dim=1)
                obj_pred = obj_pred + 1
            else:
                # NOTE: by kaihua, apply late nms for object prediction
                if boxes[0].bbox.shape[-1] == 5:
                    obj_pred = obj_prediction_nms(box.get_field('boxes_per_cls'), obj_logit, self.later_nms_pred_thres,flag = True)
                else:
            ...
            if self.use_gt_box:
                ...
                ## 
                if "HBB"  in self.type and box.extra_fields["s_f"][0] is not None:
                    w_f,h_f = box.extra_fields["s_f"][0][0],box.extra_fields["s_f"][0][1]
                    boxlist.bbox[:,0] *=   w_f
                    boxlist.bbox[:,1] *=   h_f
                    boxlist.bbox[:,2] *=   w_f
                    boxlist.bbox[:,3] *=   h_f
                elif "OBB" in  self.type and box.extra_fields["s_f"][0] is not None:
                    w_f,h_f, hw = box.extra_fields["s_f"][0][0],box.extra_fields["s_f"][0][1],box.extra_fields["s_f"][0][2]
                    boxlist.bbox[:,0] *=   w_f
                    boxlist.bbox[:,1] *=   h_f
                    boxlist.bbox[:,2:4] *= hw
