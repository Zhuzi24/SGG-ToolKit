...
class RelationSampling(object):
    ...
    def prepare_test_pairs(self, device, proposals):
        ...
        for p in proposals:
            ...
            if (not self.use_gt_box) and self.test_overlap:   #### SGDet # 0206 not perform so no change here
    ...
    def gtbox_relsample(self, proposals, targets):
        ...
        num_pos_per_img = self.mbs #int(self.batch_size_per_image * self.positive_fraction) ### nums need to be 4000 512 #
        rel_idx_pairs = []
        rel_labels = []
        rel_sym_binarys = []
    ...
    def _modify_relation_matrix(self, tgt_rel_matrix, tgt_labels, result):
        tgt_pair_idxs = torch.nonzero(tgt_rel_matrix > 0)
        tgt_head_idxs = tgt_pair_idxs[:, 0]
        tgt_tail_idxs = tgt_pair_idxs[:, 1]
        tgt_rel_labs = tgt_rel_matrix[tgt_head_idxs, tgt_tail_idxs]

        for idx in range(tgt_pair_idxs.shape[0]):
            head_idx = tgt_head_idxs[idx]
            tail_idx = tgt_tail_idxs[idx]
            rel_label = tgt_rel_labs[idx]
            head_label = tgt_labels[head_idx].item()
            tail_label = tgt_labels[tail_idx].item()
            if (head_label, tail_label, rel_label.item()) in result:
                tgt_rel_matrix[head_idx, tail_idx] = 0
        return tgt_rel_matrix
    ...
    def detect_relsample(self, proposals, targets):
        ...
        """
        self.num_pos_per_img = self.mbs # int(self.batch_size_per_image * self.positive_fraction)  256 HBB
        rel_idx_pairs = []
        rel_labels = []
        rel_sym_binarys = []
        for img_id, (proposal, target) in enumerate(zip(proposals, targets)):
            ...
            tgt_box = target.bbox
            tgt_lab = target.get_field("labels").long().cuda()
            tgt_rel_matrix = target.get_field("relation") # [tgt, tgt]
            # IoU matching
            ...
            ### RS
            if "OBB" in self.type:
                ious = torch.tensor(box_iou_rotated(  # 533,1003
                    target.bbox.float().cuda(),
                    prp_box.float()).cpu().numpy()).cuda()
            elif "HBB" in self.type:
                ious = torch.tensor(bbox_overlaps(  # 533,1003
                    target.bbox.float().cuda(),
                    prp_box.float()).cpu().numpy()).cuda()
            else:
                ious = boxlist_iou(target, proposal)  # [tgt, prp]

            is_match = (tgt_lab[:,None] == prp_lab[None]) & (ious > self.fg_thres) # [tgt, prp] need to be done
            # Proposal self IoU to filter non-overlap
            ## RS
            if "OBB" in self.type:
                prp_self_iou = torch.tensor(box_iou_rotated(  # 533,1003
                    prp_box.float(),
                    prp_box.float()).cpu().numpy()).cuda()
            elif "HBB" in self.type:
                prp_self_iou = torch.tensor(bbox_overlaps(  # 533,1003
                    prp_box.float(),
                    prp_box.float()).cpu().numpy()).cuda()
            else:   
