...
class TransformerContext(nn.Module):
    def __init__(self, config, obj_classes, rel_classes, in_channels):
        ...
        # self.context1 = SHA_Encoder(config, 1)
        # #####
        #
        # ######
        # self.lin_edge_visual = nn.Linear(self.in_channels + 512, self.hidden_dim)
        # self.lin_edge_textual = nn.Linear(200, self.hidden_dim)
        # self.context2 = SHA_Encoder(config, 1)
        # #######
    ...
    def forward(self, roi_features, proposals, logger=None):
        ...
        #
        # obj_feats_vis, _ = self.context1(obj_pre_rep_vis, obj_pre_rep_txt, num_objs)
        # # obj_feats = obj_feats_vis
        # obj_pre_rep = obj_feats_vis
        # # num_objs = [len(p) for p in proposals]
        ...
        if self.mode == 'predcls':
            ...
            ########
            # # num_objs = [len(p) for p in proposals]
            # edge_pre_rep_vis = cat((roi_features, obj_feats), -1) # 4608
            # edge_pre_rep_vis = self.lin_edge_visual(edge_pre_rep_vis)  # 4608 -512
            #
            # edge_pre_rep_txt = self.obj_embed2(obj_labels.long()) # 200
            # edge_pre_rep_txt = self.lin_edge_textual(edge_pre_rep_txt)  # 200 512
            #
            # edge_ctx_vis, _ = self.context2(edge_pre_rep_vis, edge_pre_rep_txt, num_objs)
            #
            # edge_pre_rep = edge_ctx_vis
            # ########
