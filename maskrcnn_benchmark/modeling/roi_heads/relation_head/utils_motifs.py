...
def encode_orientedbox_info(proposals):
    """
    encode proposed box information (cx, cy, w, h,th) to 
    (w/wid, h/hei, cx/wid, cy/hei, w/h, oth, dist, w*h/(wid*hei),cx/cy)
    """
    assert proposals[0].mode == 'xywha'
    boxes_info = []
    for proposal in proposals:
        boxes = proposal.bbox
        cx, cy, w, h, th = boxes.split([1,1,1,1,1], dim=-1)
        img_size = proposal.size
        wid = img_size[0]
        hei = img_size[1]
        # oth = (th+90)*math.pi/180
        assert wid * hei != 0
