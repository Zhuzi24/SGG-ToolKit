...
def evaluate_relation_of_one_image(groundtruth, prediction, global_container, evaluator):
    ...
    local_container['gt_boxes'] = groundtruth.bbox.detach().cpu().numpy()  #convert('xyxy').bbox.detach().cpu().numpy()                   # (#gt_objs, 4)
    ...
    # about objects change
    local_container['pred_boxes'] =  prediction.bbox.detach().cpu().numpy() # prediction.convert('xyxy').bbox.detach().cpu().numpy()  # RS               # (#pred_objs, 4)
