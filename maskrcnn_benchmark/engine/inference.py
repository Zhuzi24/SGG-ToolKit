...
def compute_on_dataset(model, data_loader, device, synchronize_gather=True, timer=None ,m= None,val = None
                       ,logger = None,confu_matri_path = None,ite = None,CCM = None,vae = None, bce = None):
    model.eval()
    results_dict = {}
    ...
    for id, batch in enumerate(tqdm(data_loader)): #
        ...
        with torch.no_grad():
            ...
            if cfg.TEST.BBOX_AUG.ENABLED:
                ...
                output = model(images.to(device), targets,logger = logger, sgd_data = [imgs, tar1] if imgs is not None else None)
...
def inference(
    ...
    if load_prediction_from_cache:
        ...
        predictions = compute_on_dataset(model, data_loader, device, synchronize_gather=cfg.TEST.RELATION.SYNC_GATHER,
                                         timer=inference_timer,m = m,val =val,
                                         logger=logger, confu_matri_path=confu_matri_path, ite=ite,CCM = CCM,vae = vae,bce = bce)
