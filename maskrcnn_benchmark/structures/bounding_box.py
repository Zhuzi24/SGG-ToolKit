...
class BoxList(object):
    ...
    def __init__(self, bbox, image_size, mode="xyxy",notle90=None,RS = False ):
        ...
        if mode not in ("xyxy", "xywh", "xywha"):
            raise ValueError("mode should be 'xyxy' or 'xywh' or 'xywha' ")

        self.bbox = bbox
        
        if  notle90 is not None:
            self.notle9 = notle90
    ...
    def resize(self, size,RS=False, val= False, *args, **kwargs):
        """
        ...
        if RS:
            assert self.bbox.shape[-1] == 5 # x,y,w,h,a
            ratio_width, ratio_height = ratios
            x = self.bbox[:,0] * ratio_width
            y = self.bbox[:,1] * ratio_height
            # w = self.bbox[:,2] * ratio_width
            # h = self.bbox[:,3] * ratio_height
            w = self.bbox[:,2] * np.sqrt(ratio_width * ratio_height)
            h = self.bbox[:,3] * np.sqrt(ratio_width * ratio_height)
            a = self.bbox[:,4]
            scaled_box = torch.stack((x, y, w, h, a), dim=1)
 
            bbox = BoxList(scaled_box, size, mode="xywha")
    ...
    def __getitem__(self, item):
        ...
        for k, v in self.extra_fields.items():
            if k == "data" or k == "sf_gt_box" or k == "sf_gt_labels":
                continue
            if k in self.triplet_extra_fields:
